...
title: "PSA: MITRE ATT&CK is More Than Tactics and Techniques"
...
image: /assets/images/blog/2024-07-07/mitre-attack-structure.png
featured_image: /assets/images/blog/2024-07-07/mitre-attack-structure.png
layout: post
published: true
redirect_from:
  - 
...
The ATT&CK data is built on STIX objects and structured like so;
...
Lets break this down by looking at each ATT&CK object type individually to show how I built this diagram;

## Follow along

In this post I am going to provide queries you can use to analyse and filter MITRE ATT&CK data.
...
python3 utilities/arango_cti_processor/insert_archive_attack_enterprise.py \
    --database blog_mitre_attack_data_structure \
    --versions 15_1 && \
...
### Properties: `revoked` and `x_mitre_deprecated`

When I started in this analysis these properties left me confused for a while ([I should have read this!](https://github.com/mitre-attack/attack-stix-data/blob/master/USAGE.md#working-with-deprecated-and-revoked-objects)), so I'll start here as you'll see all my queries use the filter;
...
Lets start with `x_mitre_deprecated`.

Later in this post I will talk about how STIX `course-of-action` objects represent ATT&CK Mitigations.

This was not always the case. `course-of-action` once represented techniques. These still exist in the latest bundles, as the following search uncovers, however, note the `x_mitre_deprecated` property = `true` for all these objects.
...
Which returns results including the following objects;
...
If the `x_mitre_deprecated` property is found is an object, it will always be `true`
...
Note the `revoked` property exists but not `x_mitre_deprecated`. If the `revoked` property exists it will always be `true`.
...
_Note: You can replace `mitre_attack_enterprise_vertex_collection` with either `mitre_attack_mobile_vertex_collection` or `mitre_attack_ics_vertex_collection` to change the ATT&CK matrix. This applies for all of the following searches too._
...
To see what relationships the `x-mitre-matrix` object has to/from it, you can run the search;
...
[Tactics](https://attack.mitre.org/tactics/) (`x-mitre-tactic--`) represent the "why" of an ATT&CK technique or sub-technique. It is the adversary's tactical goal: the reason for performing an action. For example, an adversary may want to achieve credential access.

Tactics have IDs in the format: `TANNNN`.

For example, [Tactic TA0043 Reconnaissance](https://attack.mitre.org/tactics/TA0043/), which can be retrieved from the database as follows;
...
[Techniques](https://attack.mitre.org/techniques/) (`attack-pattern` with Custom Property `"x_mitre_is_subtechnique": false`) represent 'how' an adversary achieves a tactical goal by performing an action. For example, an adversary may dump credentials to achieve credential access.

Techniques have IDs in the format: `TNNNN`.

For example, [Technique T1595 Active Scanning](https://attack.mitre.org/techniques/T1595/), which can be retrieved from the database as follows;
...
Note, how `x_mitre_is_subtechnique` = `false` (indicating this is not a sub-technique, which are also represented using `attack-pattern` objects).
...
For example, [T1595.001 Scanning IP Blocks](https://attack.mitre.org/techniques/T1595/001/) is a Sub-Technique of Technique [T1595 Active Scanning](https://attack.mitre.org/techniques/T1595/).
...
This time note how `x_mitre_is_subtechnique` = `true`. This is how Techniques and Sub-Techniques can be differentiated.
...
[Mitigations](https://attack.mitre.org/mitigations/) represent security concepts and classes of technologies that can be used to prevent a technique or sub-technique from being successfully executed.

Mitigations have IDs in the format: `MNNNN`.

For example, [Mitigation M1049 - Antivirus/Antimalware](https://attack.mitre.org/mitigations/M1049/) which can be retrieved from the database as follows:
...
[Groups](https://attack.mitre.org/groups/) are sets of related intrusion activity that are tracked by a common name in the security community.

Groups have IDs in the format: `GNNNN`.

For example, [Group G0016 - APT29](https://attack.mitre.org/groups/G0016/) which can be retrieved from the DB as follows;
...
[Software](https://attack.mitre.org/software/) is a generic term for custom or commercial code, operating system utilities, open-source software, or other tools used to conduct behavior modeled in ATT&CK.
...
To search for all Malware objects in the Enterprise domain:
...
[Software](https://attack.mitre.org/software/) represented as `tool` STIX Objects represents software built for an original use that was not malicious;
...
[Campaigns](https://attack.mitre.org/campaigns/) represents activities to achieve an objective.
...
To search for all Campaign in the Enterprise domain:
...
[Data Sources](https://attack.mitre.org/datasources/) represent the various subjects/topics of information that can be collected by sensors/logs.
...
[Assets](https://attack.mitre.org/assets/) represent the devices and systems commonly found within Industrial Control System environments. Each asset object includes a mapping of technique relationships that represent the adversary actions that may target the device based on its capability and function.
